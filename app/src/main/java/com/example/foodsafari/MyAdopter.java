...
public class MyAdopter  extends RecyclerView.Adapter<RestaurantViewHolder> implements Filterable {
    ...
    public void onBindViewHolder(@NonNull RestaurantViewHolder holder, final int position) {
        ...
        Picasso.get().load(restaurantList.get(position).getRestaurantImage()).placeholder(R.drawable.logo).fit().centerCrop().into(holder.imageView);
        ...
        holder.resCardview.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                Constant.user_res_choice = restaurantList.get(position).getResPassword();
                Constant.user_choose_res_name = restaurantList.get(position).getRestaurantName();


                Intent intent=new Intent(resContext,RestaurantMenu.class);
                //intent.putExtra("Res_owner_pass",Constant.user_res_choice);
